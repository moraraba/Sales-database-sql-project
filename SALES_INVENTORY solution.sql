...
(
    SELECT TO_TIMESTAMP(AVG(EXTRACT(EPOCH FROM registration_date)))
    FROM assignment.customers
);
...
SELECT *
FROM product_qty
WHERE total_qty > (SELECT AVG(total_qty) FROM product_qty);
...
SELECT *
FROM spending
WHERE total_spent > (SELECT AVG(total_spent) FROM spending);
...
(
    SELECT AVG(total_spent)
    FROM spending s2
    WHERE s1.membership_status = s2.membership_status
);
...
(
    ...
    WHERE ps1.category = ps2.category
);
...
ratios AS (
    SELECT customer_id,
           ...
           total_amount,
           total_spent,
           total_amount / total_spent AS ratio
